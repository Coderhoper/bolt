    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="lg:pl-64">
        ...
        <main className="p-4 lg:p-8">
          ...
          {successNotice && <div role="status" className="mb-5 flex items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800"><BadgeCheck size={17} className="mt-0.5 shrink-0" /><span>{successNotice}</span><button className="ml-auto" aria-label="Dismiss notice" onClick={() => setSuccessNotice('')}><X size={16} /></button></div>}
          ...
          {page === 'tenants' && <Tenants tenants={filteredTenants} search={search} setSearch={setSearch} onCreate={() => setShowCreate(true)} onProvision={prepareProvisioning} canProvision={canProvision} busyTenantId={provisioningTenantId} />}
...
function TenantRow({ tenant }: { tenant: Tenant }) { const tone = tenantIsProvisioned(tenant) ? 'bg-emerald-50 text-emerald-700' : tenant.status === 'attention' ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-800'; return <div className="flex items-center gap-3 px-5 py-3.5"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-600"><Building2 size={17} /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{tenant.name}</p><p className="truncate text-xs text-slate-500">{tenant.slug} · {pretty(tenant.plan)}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${tone}`}>{tenantIsProvisioned(tenant) ? 'Active' : tenant.status === 'active' ? 'Setup required' : pretty(tenant.status)}</span></div>; }
...
function Tenants({ tenants, search, setSearch, onCreate, onProvision, canProvision, busyTenantId }: { tenants: Tenant[]; search: string; setSearch: (v: string) => void; onCreate: () => void; onProvision: (tenant: Tenant) => void; canProvision: boolean; busyTenantId: string }) {
  return <>
    <PageHeading eyebrow="Tenant registry" title="Tenants" subtitle="Register, provision and manage isolated customer environments." action={canProvision ? <button onClick={onCreate} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"><Plus size={17} /> Add tenant</button> : undefined} />
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[920px] text-left text-sm">
      <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr>{['Tenant', 'Plan', 'Region', 'Admin contact', 'Isolation', 'Status', 'Created', 'Access'].map(h => <th key={h} className="px-4 py-3 font-semibold">{h}</th>)}</tr></thead>
      <tbody className="divide-y divide-slate-100">{tenants.map(t => <tr key={t.id} className="hover:bg-slate-50">
        <td className="px-4 py-3"><p className="font-medium text-slate-900">{t.name}</p><p className="text-xs text-slate-500">{t.slug}</p>{t.provisioning_step && t.status !== 'active' && <p className="mt-1 text-xs text-blue-700">{pretty(t.provisioning_step)}</p>}</td>
        <td className="px-4 py-3">{pretty(t.plan)}</td><td className="px-4 py-3">{pretty(t.region)}</td>
        <td className="px-4 py-3"><p>{t.primary_contact || '?'}</p><p className="text-xs text-slate-500">{t.contact_email || 'Admin email required'}</p></td>
        <td className="px-4 py-3 text-xs">{pretty(t.isolation_level)}</td>
        <td className="px-4 py-3"><span className={['rounded-full px-2.5 py-1 text-xs', tenantIsProvisioned(t) ? 'bg-emerald-50 text-emerald-700' : t.status === 'attention' ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-800'].join(' ')}>{tenantIsProvisioned(t) ? 'Active' : t.status === 'active' ? 'Setup required' : pretty(t.status)}</span></td>
        <td className="px-4 py-3 text-slate-500">{dateLabel(t.created_at)}</td>
        <td className="px-4 py-3">{tenantIsProvisioned(t) ? <a href={'/t/' + t.slug} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm font-medium text-blue-700">Open tenant<ArrowUpRight size={14} /></a> : canProvision ? <button type="button" disabled={busyTenantId === t.id} onClick={() => onProvision(t)} className="rounded-lg border border-blue-200 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-50 disabled:opacity-50">{busyTenantId === t.id ? 'Working?' : t.status === 'provisioning' || t.status === 'attention' ? 'Resume setup' : 'Provision'}</button> : <span className="text-xs text-slate-400">Setup required</span>}</td>
      </tr>)}</tbody>
...
function Onboarding({ tenants, jobs, onCreate, onProvision, canProvision, busyTenantId }: { tenants: Tenant[]; jobs: FeedRow[]; onCreate: () => void; onProvision: (tenant: Tenant) => void; canProvision: boolean; busyTenantId: string }) {
  ...
  return <>
    <PageHeading eyebrow="Tenant lifecycle" title="Onboarding pipeline" subtitle="Register the customer, provision its isolated database, then invite its administrator." action={canProvision ? <button onClick={onCreate} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white"><Plus size={17} /> Register tenant</button> : undefined} />
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{stages.map((stage, i) => { const count = tenants.filter(t => stage === 'active' ? tenantIsProvisioned(t) : t.status === stage || (stage === 'pending' && (['trial', 'contracted'].includes(t.status) || (t.status === 'active' && !tenantIsProvisioned(t))))).length; return <div key={stage} className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><span className="text-sm font-medium text-slate-600">{pretty(stage)}</span><span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-50 text-xs font-bold text-blue-700">{i + 1}</span></div><p className="mt-3 text-3xl font-bold">{count}</p><p className="mt-1 text-xs text-slate-500">{i === 0 ? 'Contract signed, awaiting setup' : i === 1 ? 'Isolated environment setup' : i === 2 ? 'Tenant admin course progress' : 'Health checks active'}</p></div>; })}</div>
    <div className="mt-6 grid gap-5 xl:grid-cols-[1.2fr_1fr]"><div className="rounded-xl border border-slate-200 bg-white"><div className="border-b border-slate-100 p-5"><h2 className="font-semibold">Tenant onboarding</h2><p className="mt-1 text-sm text-slate-500">Each tenant receives its own database and a sign-in invite for the first administrator.</p></div>{onboardingTenants.length ? <div className="divide-y divide-slate-100">{onboardingTenants.map(t => <div key={t.id} className="flex items-center gap-3 py-1"><div className="min-w-0 flex-1"><TenantRow tenant={t} /></div>{canProvision && <button type="button" disabled={busyTenantId === t.id} onClick={() => onProvision(t)} className="mr-4 shrink-0 rounded-lg border border-blue-200 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-50 disabled:opacity-50">{busyTenantId === t.id ? 'Working?' : t.status === 'provisioning' || t.status === 'attention' ? 'Resume' : 'Provision'}</button>}</div>)}</div> : <Empty title="Pipeline is clear" text="Newly registered tenants will appear here." />}</div>
      <div className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="font-semibold">Provisioning jobs</h2><p className="mt-1 text-sm text-slate-500">Durable job state; retries resume at the last committed step.</p>{jobs.length ? <div className="mt-4 space-y-3">{jobs.map(job => { const tenant = tenants.find(t => t.id === job.tenant_id); return <div key={job.id} className="flex items-center gap-3 rounded-lg bg-slate-50 p-3"><Database size={17} className="text-blue-700" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{tenant?.name || 'Tenant environment'}</p><p className="text-xs text-slate-500">{pretty(job.status)} ? {pretty(String(job.current_step || 'queued'))} ? {dateLabel(job.created_at)}</p></div>{tenant && !tenantIsProvisioned(tenant) && canProvision && <button type="button" disabled={busyTenantId === tenant.id} onClick={() => onProvision(tenant)} className="rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs font-semibold disabled:opacity-50">Resume</button>}</div>; })}</div> : <div className="mt-5 rounded-lg border border-dashed border-slate-300 p-4 text-sm text-slate-500">No onboarding jobs are queued.</div>}</div>
    </div>
  </>;
}
...
function Platform({ jobs, tenants, managementCheck, onCheckManagement }: { jobs: FeedRow[]; tenants: Tenant[]; managementCheck: { loading: boolean; message: string; ok: boolean | null }; onCheckManagement: () => void }) { return <><PageHeading eyebrow="Platform control" title="Platform operations" subtitle="Provisioning, release, feature flags, backups and system audit surfaces." /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Provisioning jobs" value={jobs.length} detail="Recent owner-plane jobs" icon={Database} /><Stat label="Registered environments" value={tenants.length} detail="Metadata registry records" icon={Building2} tone="emerald" /><Stat label="Release management" value="Not connected" detail="No deployment control API configured" icon={RefreshCw} tone="amber" /><Stat label="Feature flags" value="Not connected" detail="Flags must be delivered through a secured service" icon={Settings2} tone="slate" /></div><div className="mt-6 rounded-xl border border-slate-200 bg-white p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold">Supabase organization connection</h2><p className="mt-1 text-sm text-slate-500">Checks read access to the configured organization and reports its project count. This check never creates or changes a project.</p></div><button type="button" onClick={onCheckManagement} disabled={managementCheck.loading} className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"><RefreshCw size={15} className={managementCheck.loading ? 'animate-spin' : ''} />{managementCheck.loading ? 'Checking…' : 'Check connection'}</button></div>{managementCheck.message && <p role="status" className={`mt-3 rounded-lg p-3 text-sm ${managementCheck.ok ? 'bg-emerald-50 text-emerald-800' : 'bg-rose-50 text-rose-800'}`}>{managementCheck.message}</p>}</div><div className="mt-6 grid gap-4 md:grid-cols-2"><PlatformCard title="Provisioning" text="Creates one isolated tenant project, applies the checked-in schema and catalogue migrations, configures slug-based sign-in, and invites the tenant administrator. Jobs can be resumed after a failed step." icon={Database} /><PlatformCard title="Flags & releases" text="Keep releases and tenant feature configuration behind an authenticated server-side service. The owner UI does not hold deployment credentials." icon={Settings2} /><PlatformCard title="Backup & restore" text="Per-tenant encrypted backups and tested restore workflows belong to the infrastructure plane. Never expose raw tenant backup contents here." icon={ShieldCheck} /><PlatformCard title="Tenant trust" text="Tenant projects are separate databases; the owner plane receives only metadata and the active project's browser-safe URL and publishable key." icon={Users} /></div></>; }
...
function TenantModal({ form, setForm, onClose, onSubmit, busy }: { form: { name: string; slug: string; plan: string; region: string; primary_contact: string; contact_email: string; isolation_level: string }; setForm: (v: { name: string; slug: string; plan: string; region: string; primary_contact: string; contact_email: string; isolation_level: string }) => void; onClose: () => void; onSubmit: (e: FormEvent) => void; busy: boolean }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4"><form onSubmit={onSubmit} className="max-h-[95vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
    <div className="flex items-start justify-between border-b border-slate-100 p-5"><div><h2 className="text-lg font-bold">Register tenant</h2><p className="mt-1 text-sm text-slate-500">Set up the customer record and first administrator invite.</p></div><button type="button" onClick={onClose} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100" aria-label="Close"><X size={19} /></button></div>
    <div className="grid gap-4 p-5 sm:grid-cols-2">{([['name', 'Company name'], ['slug', 'Tenant slug'], ['primary_contact', 'Primary contact'], ['contact_email', 'First tenant admin email']] as [keyof typeof form, string][]).map(([key, label]) => <label key={key} className="text-sm font-medium">{label}<input required={key !== 'primary_contact'} type={key === 'contact_email' ? 'email' : 'text'} autoComplete={key === 'contact_email' ? 'email' : undefined} value={form[key]} onChange={e => update(key, e.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-blue-500" /></label>)}
      <label className="text-sm font-medium">Plan<select value={form.plan} onChange={e => update('plan', e.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2"><option value="starter">Starter</option><option value="growth">Growth</option><option value="enterprise">Enterprise</option></select></label>
      <label className="text-sm font-medium">Region<select value={form.region} onChange={e => update('region', e.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2"><option value="africa-east">East Africa ? Mumbai</option><option value="africa-south">Southern Africa ? Ireland</option><option value="eu-west">Europe West ? Ireland</option><option value="us-east">US East ? Virginia</option></select></label>
      <div className="sm:col-span-2 rounded-lg bg-blue-50 p-3 text-xs leading-5 text-blue-900">Each tenant is provisioned into a separate Supabase database with the tenant schema and hardware catalogue. The first admin receives an invitation link for this tenant?s sign-in page.</div>
    </div>
    <div className="flex justify-end gap-2 border-t border-slate-100 p-5"><button type="button" onClick={onClose} className="rounded-lg border border-slate-300 px-4 py-2 text-sm">Cancel</button><button disabled={busy} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{busy ? 'Registering?' : <><Check size={16} /> Create tenant</>}</button></div>
  </form></div>;
}
