-- Sales must reference active, catalogue-backed inventory. Prices and stock are
-- read and changed on the server so clients cannot submit arbitrary products or prices.

CREATE OR REPLACE FUNCTION public.enforce_catalog_sale_item()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Sale items must reference active catalogue products';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS sale_items_require_catalog_product ON public.sale_items;
CREATE TRIGGER sale_items_require_catalog_product
  BEFORE INSERT OR UPDATE OF product_id ON public.sale_items
  FOR EACH ROW EXECUTE FUNCTION public.enforce_catalog_sale_item();

REVOKE ALL ON FUNCTION public.enforce_catalog_sale_item() FROM PUBLIC, anon, authenticated;
...
COMMENT ON FUNCTION public.process_sale(jsonb, text, text, text, date)
  IS 'Records sales from active catalogue-linked inventory, deriving prices and stock server-side.';
